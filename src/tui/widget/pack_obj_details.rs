...
pub enum PackObjectWidget {
    ...
    Initiolized {
        ...
        text_cache: Option<(ratatui::text::Text<'static>, usize)>, // (content, line_count)
    },
}
...
impl PackObjectWidget {
    ...
    pub fn text(&mut self) -> ratatui::text::Text<'static> {
        if let Some((cached_content, _)) = &self.text_cache {
            // Return a clone of the cached content
            return cached_content.clone();
        }

        // Generate content and cache it
        let content = generate_pack_object_detail_content(&self.pack_obj);
        let line_count = content.lines.len();
        self.text_cache = Some((content.clone(), line_count));
        content
    }
    ...
    pub fn text_len(&mut self) -> usize {
        match self {
            Self::Initiolized {
                ...
                if let Some((_, cached_line_count)) = text_cache {
                    // Return a clone of the cached content
                ...
                let line_count = content.lines.len();
                *text_cache = Some((content.clone(), line_count));
                line_count
            }
    ...
    pub fn set_scroll_position(&mut self, new_scroll_position: usize) {
        match self {
            Self::Initiolized {
                scroll_position, ..
            } => {
    ...
    pub fn render(&self, f: &mut ratatui::Frame, area: ratatui::layout::Rect, is_focused: bool) {
        // let pack_object = self.pack_obj;
        // let scroll_position = self.scroll_position,
        // let cache: Option<&Option<(ratatui::text::Text<'static>, usize)>>,
