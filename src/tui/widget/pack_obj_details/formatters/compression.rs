...
impl<'a> ZlibHeaderFormatter<'a> {
    ...
    fn format_flag_explanations(
        ...
        lines.push(Line::from(vec![
            ...
            Span::from("FCHECK: "),
            Span::styled(format!("{:05b}", fcheck), Style::default().fg(COLORS[2])),
...
impl<'a> DeflateBlockFormatter<'a> {
    ...
    pub fn format_block_header(&self, lines: &mut Vec<Line<'static>>) {
        ...
        let deflate_line = vec![
            ...
            Span::styled(format!("{:02b}", btype), Style::default().fg(COLORS[0])),
            Span::styled(format!("{:01b}", bfinal), Style::default().fg(COLORS[1])),
            Span::from("┊"),
            Span::from(" - 0x"),
            Span::from(format!("{:02X}", byte3)),
        ];
